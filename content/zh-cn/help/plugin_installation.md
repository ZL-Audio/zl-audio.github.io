...
在 Linux 系统上，插件不支持硬件加速。插件是在 Ubuntu 24.04 上构建的，带有动态链接依赖。因此，我无法保证其与其他 Linux 发行版的兼容性。我建议您从源码编译该插件。

{{% alert title="Warning" color="warning" %}}
在可以预见的未来（Ubuntu 28.04 发布后），插件将在 Ubuntu 26.04 上构建，这将导致其与之前的 Linux 版本不兼容。
{{% /alert %}}
