...
> 自动化调制速度不低于 1000 次每秒，而不是每个音频样本均进行改变。
> 
> 在插件界面打开时调制滤波器参数，会给插件的界面渲染带来较大压力。因此可能导致宿主界面出现冻结。
>
> 插件内部对于调制不进行任何平滑处理，不平滑的调制 或者 调制 滤波器种类、斜率、立体声设置 等会导致滤波器产生噪声。
>
> 当您已经调制多个频段的滤波器参数时，请勿在频谱图同时框选这些频段。
