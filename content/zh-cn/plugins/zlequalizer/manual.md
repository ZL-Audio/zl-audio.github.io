...
>
> 冲突检测使用独立的频谱图，因此 `Analyzer` 设置将不会影响冲突检测。
>
> 冲突检测打开后，`Main Panel` 的帧率将降低以控制插件界面渲染压力。
> 
> 冲突检测可能引起界面卡顿。
...
- `Pre`、`Post`、`Side` 分别代表输入主链信号、输出主链信号和侧链信号
- `OFF`：关闭该信号频谱图
- `ON`：打开该信号频谱图
- `FRZ`：冻结该信号频谱图
- 可以通过单击左侧标签快速切换
...
- 按下：取消旁路当前所选频段的[动态](#动态与自动阈值)效果
- 弹起：旁路当前所选频段的[动态](#动态与自动阈值)效果
...
- `Side`: 从侧链信号学习（若要学习外部侧链，确保您在[右上方按钮](#右上方按钮)中打开了外部侧链）
...
当选中多个频段时，旁路/关闭其中一个频段 或 调整其中一个频段的频率/增益/带宽，也将影响其他选中频段的对应参数。
...
> 自动化调制速度为每个音频缓冲块一次。但是，滤波器参数（`Freq`、`Gain` 和 `Q`）的调制会每个音频样本改变滤波器状态。
> 
...
3. 调整差值。通过调节 `Smooth`、调节 `Slope`、拖拽偏移按钮或手动绘制等方式来调整当前的差值曲线。
