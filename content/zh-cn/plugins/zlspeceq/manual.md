...
不同于基于宽带音频触发器应用宽滤波器曲线的 ZL Equalizer 2，ZL Spectrum Equalizer 使用快速傅里叶变换直接在频域中操作。音频被分割成离散的频率仓，从而允许根据相应侧链仓的电平，独立地对每个仓应用动态增益调节。
...
- **Smooth Type（平滑类型）**：选择频谱处理侧链平滑类型。
- **Smooth Value（平滑值）**：调节频谱处理侧链平滑值。更高的值会让动态处理在频域上更平滑。
