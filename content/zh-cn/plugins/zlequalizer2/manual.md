...
- **多样的功能**：通过 6 种滤波器结构、10 种滤波器类型、5 种立体声模式、7 种可变斜率和多达 24 个频段来雕刻您的声音。
- **丰富的动态**：超越静态 EQ，具有可调的阈值、启动时间、释放时间和侧链滤波器，可实现强大的动态均衡。
- **纯净的精度**：64 位浮点处理和先进的去扭曲技术提供了卓越的性能，确保了从低频到高频都有出色的清晰度。
...
选择滤波器类型：`峰值 (Peak)`、`低搁架 (Low Shelf)`、`低通 (Low Pass)`、`高搁架 (High Shelf)`、`高通 (High Pass)`、`带阻 (Notch)`、`带通 (Band Pass)`、`倾斜搁架 (Tilt Shelf)` 和 `平坦倾斜 (Flat Tilt)`。

---

**Slope (斜率)**

选择滤波器斜率：`6 dB/oct`、`12 dB/oct`、`24 dB/oct`、`36 dB/oct`、`48 dB/oct`、`72 dB/oct` 和 `96 dB/oct`。较高的斜率会使滤波器的响应曲线变化更陡峭。`峰值 (Peak)`、`带阻 (Notch)` 和 `带通 (Band Pass)` 不支持 `6 dB/oct`。`平坦倾斜 (Flat Tilt)` 不支持斜率设置。
...
- 释放：关闭动态学习行为，并设置 `Threshold` (阈值) 与 `Knee` (拐点)。
...
- 释放：关闭 AGC，并更新 `Output Gain` (输出增益) 为两者响度差值
...
- 控制平滑值，数值越大，频谱越平滑
- `OCT`：按八度音阶 (octave) 对频谱进行平滑
- `ERB`：按等效矩形带宽 (ERB, Equivalent Rectangular Bandwidth) 对频谱进行平滑
...
- `Balanced (均衡)`：将目标曲线设置为一条均衡的线
- `Natural (自然)`：将目标曲线设置为一条自然的线
...
- 您可以在按下 `Ctrl/Command` 的同时用鼠标右键拖动，以固定增益。
