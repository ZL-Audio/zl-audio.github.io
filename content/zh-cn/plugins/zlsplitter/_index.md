...
> 在 macOS 系统上，该插件需要 Metal 支持。由于某些未知原因，该插件可能无法在 macOS 11 之前的系统上运行。除此以外，如果您对安装过程中的警告信息感到困惑，请参阅[在 Mac 上安全地打开 App](https://support.apple.com/zh-cn/102445)。
>
...
> 当滤波器为 SVF 结构，该滤波器会产生显著的相位改变。在这种情况下，不应当将输出与原始信号直接混合 或者 级联多个 低频/高频分离 效果。

> 当滤波器为 FIR 结构，该滤波器的不会产生相位改变，但插件会产生延时。延时的具体大小取决于采样率和滤波器斜率。
...
- 平衡为 -50 时，瞬态包含信号最少，稳态包含信号最多
- 平衡为 50 时，瞬态包含信号最多，稳态包含信号最少。

___

**保持（Hold）**

控制瞬态/稳态分离的保持。保持越大，瞬态衰减速度越慢。
___
...
- 频谱平滑为 0 时，信号不同频率的部分相对独立地进行瞬态/稳态分离
...
改变 文字颜色 和 背景颜色 会**大幅度**改变用户界面的颜色。为了良好的显示效果，请将两者设置为对比度高的颜色。
